...
function shouldUnfold(s){
  return isVernacStart(coqdocjs.transparent, s);
}
...
function foldProofs() {
  ...
  toArray(nodes).forEach(function(node){
    if(isProofStart(node)) {
	  ...
	  if (shouldUnfold(proofEndText)) { proof.setAttribute("show", "true"); }
      else { proof.setAttribute("show", "false"); }
    }
  });
}
...
function repairNotations() {
	// For some reason, backslashes are introduced as standalone notations.
    // We append the backslash to the next node.
	toArray(document.getElementsByClassName("id")).forEach(function(node){
		const t = node.getAttribute("type");
		if (t === "notation" && node.firstChild.textContent == "\\") {
			const actualNotation = node.parentNode.nextSibling.firstChild;
			actualNotation.firstChild.textContent = "\\" + actualNotation.firstChild.textContent;
